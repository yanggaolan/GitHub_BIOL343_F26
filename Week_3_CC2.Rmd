...
1. **Name** — student number
    + [BIOL343_CC](https://github.com/your_user_name/BIOL343_CC)
1. **Julia** - 123456
    + [GitHub] (https://github.com/yanggaolan/GitHub_BIOL343_F26.git)
...
----------------|-----------------|------------
population_type | population      | numbers
ovary_width     | ovary width     | cm
ovary_height    | ovary height    | cm
flower_number   | flower number   | counts
...
area_means <- aggregate(ovary_area ~ population_type, data = cc1_data, mean)
```
...
```{r area-table, echo = F}
kable(area_means, caption = "area means")
```
...
```{r area-histogram}
ggplot(aes(x = ovary_area), data = cc1_data) +
    geom_histogram(binwidth = 3) +
    facet_wrap(.~population_type) +
    labs(title = "Ovary area",
    x = "Ovary area",
    y = "Counts")
...
![*Solanum pimpinellifolium*](./image/Hydroponic_g13.jpg)
